...
type LandingPageProps = { onStart: () => void; onTryDemo?: () => void; heroCompanion?: ReactNode };
...
export function LandingPage({ onStart, onTryDemo, heroCompanion }: LandingPageProps) {
  const tryDemo = onTryDemo ?? onStart;
  return <>
    ...
    <section aria-labelledby="judge-preview-title" className="px-5 py-20 md:px-10"><div className="mx-auto grid max-w-7xl gap-7 overflow-hidden rounded-3xl border border-[#9c8aff]/25 bg-[linear-gradient(120deg,rgb(33_27_82_/_80%),rgb(11_16_45_/_88%))] p-6 md:p-9 lg:grid-cols-[.85fr_1.15fr] lg:p-12"><div><p className="text-xs font-medium tracking-[.18em] text-[#b9adff]">JUDGE DEMO</p><h2 id="judge-preview-title" className="mt-4 text-3xl font-semibold tracking-tight text-white md:text-4xl">Experience TrustDNA in action.</h2><p className="mt-4 max-w-md text-sm leading-6 text-slate-300">No login. No setup. Choose a prepared case and watch Sentinel turn artifact signals into an explainable decision.</p><Button onClick={tryDemo} className="mt-7 h-11 rounded-xl bg-white px-5 text-[#17102f] hover:bg-[#ece9ff]">Try Judge Demo <ArrowRight className="size-4" /></Button></div><div className="grid gap-3 sm:grid-cols-2">{[{ icon: MailWarning, title: "Fake CEO Email", tag: "EMAIL" }, { icon: AudioLines, title: "Cloned Voice", tag: "VOICE" }, { icon: FileWarning, title: "Forged Resume", tag: "DOCUMENT" }, { icon: ShieldAlert, title: "Fake Certificate", tag: "CREDENTIAL" }].map(({ icon: Icon, title, tag }) => <button type="button" onClick={tryDemo} className="rounded-2xl border border-white/[.1] bg-white/[.035] p-5 text-left transition hover:-translate-y-1 hover:border-[#c0b4ff]/45 hover:bg-white/[.06] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#c0b4ff]" key={title}><Icon aria-hidden="true" className="size-5 text-[#c0b4ff]" /><p className="mt-7 text-sm font-medium text-white">{title}</p><p className="mt-2 font-mono text-[10px] tracking-[.14em] text-slate-500">{tag}</p></button>)}</div></div></section>
