...
export function TrustDNAExperience() {
  ...
  function goToJudgeMode() { document.getElementById("judge-mode")?.scrollIntoView({ behavior: "smooth", block: "start" }); }
  ...
  return <main className="app-backdrop min-h-screen overflow-hidden"><div className="grid-overlay pointer-events-none fixed inset-0 opacity-50" /><div className="relative"><LandingPage onStart={goToJudgeMode} /><JudgeMode selected={selected} onSelect={setSelected} onStart={startInvestigation} disabled={running} /><AnimatePresence>{(running || error || result) && <motion.div id="live-investigation" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>{running && <InvestigationConsole activeStep={activeStep} />}{error && <div className="mx-auto max-w-4xl px-5 pb-8 md:px-10"><div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-red-300/20 bg-red-400/[.08] p-5 text-red-100"><div className="flex items-center gap-3"><AlertCircle className="size-5" /><p className="text-sm">{error}</p></div><Button onClick={startInvestigation} variant="outline" className="border-red-200/25 bg-transparent text-red-100 hover:bg-red-100/10 hover:text-white">Retry investigation</Button></div></div>}</motion.div>}</AnimatePresence>{result && <motion.div id="case-file" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }}><CaseFile result={result} scenario={selected} /><TrustCertificate result={result} /><EvidenceReport result={result} /></motion.div>}<section className="mx-auto max-w-7xl px-5 pb-20 pt-12 md:px-10"><div className="grid gap-5 border-t border-white/[.08] pt-12 md:grid-cols-[.8fr_1.2fr]"><div><p className="text-xs font-medium tracking-[.18em] text-[#aaa0ff]">HOW TRUSTDNA WORKS</p><h2 className="mt-3 text-2xl font-semibold text-white">Identity Genome → Evidence → Trust</h2></div><div className="grid gap-4 sm:grid-cols-3">{[{ icon: ShieldCheck, label: "Identity Genome", body: "Versioned, explainable traits—not a black box." }, { icon: Cpu, label: "Forensic agents", body: "Structured evidence flows through Sentinel." }, { icon: ShieldCheck, label: "Trust credential", body: "A certificate is created from the Case File." }].map(({ icon: Icon, label, body }) => <div className="rounded-2xl border border-white/[.07] bg-white/[.025] p-5" key={label}><Icon className="size-5 text-[#ad9fff]" /><p className="mt-4 text-sm font-medium text-slate-100">{label}</p><p className="mt-2 text-xs leading-5 text-slate-500">{body}</p></div>)}</div></div></section></div></main>;
}
