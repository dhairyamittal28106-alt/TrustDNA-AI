import type { BehaviorPattern, IdentityDecision, ReasoningContext, ReasoningEvidence } from "@/features/identity-reasoning/types";

type DecisionInput = Pick<ReasoningContext, "intent" | "dimensions" | "behaviorSignals"> & {
...
export class IdentityDecisionEngine {
  decide({ intent, dimensions, behaviorSignals, evidence }: DecisionInput): IdentityDecision {
    const values = new Map(dimensions.map((dimension) => [dimension.id, dimension.value]));
    const available = this.withBehavior(evidence.map((item) => `${item.title}: ${item.value}`), behaviorSignals);
    ...
    switch (intent) {
      ...
      case "motivation":
        return this.withEvidence("Motivation evidence", this.withBehavior(this.pick(values, ["goals", "dreams", "motivations", "values", "career", "interests"]), behaviorSignals), {
          recommendation: values.has("goals") || values.has("dreams") || values.has("motivations") ? "Use the stated goals, dreams, and career aims as the clearest current motivation signals; validate them against your next concrete decision." : "Add a direct statement about what matters to you before relying on a motivation recommendation.",
      ...
      case "strengths":
        return this.withEvidence("Evidence-backed strengths", this.withBehavior(this.pick(values, ["strengths", "projects", "skills", "frameworks", "communication"]), behaviorSignals), {
      ...
      case "career":
        return this.withEvidence("Career-direction evidence", this.withBehavior(this.pick(values, ["goals", "dreams", "motivations", "values", "career", "projects", "strengths", "skills", "education"]), behaviorSignals), {
      ...
      case "management":
        return this.withEvidence("Management-fit evidence", this.withBehavior(this.pick(values, ["projects", "strengths", "communication", "career", "goals", "values"]), behaviorSignals), {
      ...
      case "entrepreneurship":
        return this.withEvidence("Entrepreneurship-fit evidence", this.withBehavior(this.pick(values, ["goals", "dreams", "motivations", "values", "projects", "career", "strengths", "skills"]), behaviorSignals), {
          recommendation: values.has("projects") || values.has("goals") || values.has("dreams") || values.has("motivations") ? "The recorded goals, dreams, motivations, and project evidence support exploring product-building through a small, time-bounded experiment. They do not establish business readiness or risk tolerance." : "Add direct goals, project evidence, or career preferences before assessing entrepreneurship alignment.",
      ...
      case "higher_studies":
        return this.withEvidence("Higher-studies evidence", this.withBehavior(this.pick(values, ["education", "goals", "dreams", "motivations", "career", "skills", "values"]), behaviorSignals), {
      ...
      case "startup_fit":
        return this.withEvidence("Startup-fit evidence", this.withBehavior(this.pick(values, ["goals", "dreams", "motivations", "values", "projects", "career", "strengths", "skills"]), behaviorSignals), {
      ...
      case "goal_alignment":
        return this.withEvidence("Goal-alignment evidence", this.withBehavior(this.pick(values, ["goals", "dreams", "motivations", "values", "career", "projects", "interests"]), behaviorSignals), {
      ...
      case "general_decision":
        return this.withEvidence("Decision-support evidence", this.withBehavior(this.pick(values, ["goals", "dreams", "motivations", "values", "career", "projects", "strengths", "interests", "education"]), behaviorSignals), {
  ...
  private withBehavior(evidence: string[], signals: BehaviorPattern[]): string[] {
    return [...evidence, ...signals.map((signal) => `Behavior evidence: ${signal.label}`)];
  }
}
