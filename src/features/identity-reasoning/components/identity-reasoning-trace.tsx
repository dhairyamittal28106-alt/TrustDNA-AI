...
import { IdentityProfilePanel } from "@/features/identity-reasoning/components/identity-profile-panel";
import { DigitalBrain } from "@/features/identity-reasoning/components/digital-brain";
...
export function IdentityReasoningTrace({ reasoning }: IdentityReasoningTraceProps) {
  ...
  const goalsUsed = reasoning.dimensions.filter((dimension) => dimension.id === "goals" || dimension.id === "dreams").map((dimension) => dimension.label).join(" · ");
  const valuesUsed = reasoning.dimensions.filter((dimension) => dimension.id === "values").map((dimension) => dimension.label).join(" · ");
  return <motion.section initial={{ opacity: 0, y: reduceMotion ? 0 : 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : .35 }} aria-labelledby="identity-reasoning-heading" className="mt-5 space-y-5">
    ...
    <DigitalBrain reasoning={reasoning} />
    <IdentityProfilePanel profile={reasoning.profile} usedDimensionIds={reasoning.dimensions.map((dimension) => dimension.id)} usedBehaviorIds={reasoning.behaviorPatterns.map((pattern) => pattern.id)} />
    <section aria-label="Reasoning context used" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><ReasoningContextMetric label="Goals used" value={goalsUsed || "No goal evidence selected"} /><ReasoningContextMetric label="Values used" value={valuesUsed || "No value evidence selected"} /><ReasoningContextMetric label="Behavior signals used" value={reasoning.behaviorPatterns.length ? `${reasoning.behaviorPatterns.length} deterministic signal${reasoning.behaviorPatterns.length === 1 ? "" : "s"}` : "No repeated signal selected"} /><ReasoningContextMetric label="Evidence used" value={`${reasoning.evidence.length} source-linked dimension${reasoning.evidence.length === 1 ? "" : "s"}`} /></section>
...
function ReasoningContextMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-white/[.08] bg-black/[.12] px-4 py-3"><p className="font-mono text-[8px] tracking-[.1em] text-slate-600">{label.toUpperCase()}</p><p className="mt-1 text-xs leading-5 text-slate-300">{value}</p></div>;
}
