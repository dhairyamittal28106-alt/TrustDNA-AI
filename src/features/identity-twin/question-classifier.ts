...
const identityFactPattern = /\b(?:name|who\s+am\s+i|date\s+of\s+birth|birthday|when\s+was\s+i\s+born|dream|ambition|university|college|school|degree|branch|department|major|speciali[sz]ation|education|academic\s+history|favorite|favourite|cricketer|player|team|project|projects|skill|skills|technology|technologies|goal|goals|current\s+role|timeline|what\s+motivates\s+me|my\s+values|interests)\b/i;
